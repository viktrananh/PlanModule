...
{
    internal class LineAppointmentDAO : BaseDAO
    {

        public List<LineControl> GetAllLines()
        {
            List<LineControl> lines = new List<LineControl>();
            DataTable dt = mySQL.GetDataMySQL($"SELECT * FROM PRODUCTION_TRACER.LINE_CONTROL;");
            if (istableNull(dt)) return new List<LineControl>();
            lines = (from r in dt.AsEnumerable()
                     select new LineControl()
                     {
                         LineName = r.Field<string>("ROUTE_NAME") + "-" + r.Field<string>("LINE_NAME"),
                         RouterName = r.Field<string>("ROUTE_NAME")
                     }).ToList();
            return lines;
        }
