...
namespace PLAN_MODULE.DAO.PLAN
{
    internal class BillExportGoodsDAO : BaseDAO
    {
        ...
        {
            string cmd = $"SELECT sum(pcbs) FROM TRACKING_SYSTEM.HISTORY_BOX_EXPORT where WORK_ID='{workID}';";
        ...
        public string createBillRequest(DateTime dtExport)
        {
            ...
            string sql = $"SELECT * FROM STORE_MATERIAL_DB.BILL_REQUEST_EX where date(DATE_EXPORT) =  date('{dtExport.ToString("yyyy-MM-dd HH:mm:ss")}') AND TYPE_BILL = 6 ORDER BY BILL_NUMBER DESC;";
            DataTable dt = DBConnect.getData(sql);
            if (istableNull(dt))
            {
        ...
        public DataTable getDetailBill(string billNumber)
        {
            ...
            string sql = $"SELECT WORK_ID, CUS_MODEL, CUS_CODE, MODEL, UNIT,PO, NUMBER_REQUEST FROM TRACKING_SYSTEM.DELIVERY_BILL where BILL_NUMBER = '{billNumber}';";
            DataTable dt = DBConnect.getData(sql);
        ...
        {
            DataTable DT = mySQL.GetDataMySQL($"SELECT * FROM TRACKING_SYSTEM.HISTORY_BOX_EXPORT where BILL_NUMBER='{bill}';");
        ...
        {
            DataTable dt = mySQL.GetDataMySQL($"SELECT  A.WORK_ID, B.MODEL, A.REQUEST, A.BOX_COUNT, A.REQUEST_AFTER , A.BOX_SERIAL, A.LOT_NO, B.STATE  FROM TRACKING_SYSTEM.FP_BOOK_BILL_DETAIL A INNER JOIN TRACKING_SYSTEM.BOX_LIST B ON A.BOX_SERIAL = B.BOX_SERIAL WHERE BILL_NUMBER = '{bill}' ;");
            if (istableNull(dt)) return null;
