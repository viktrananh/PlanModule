...
namespace PLAN_MODULE.BUS.Sales
{
    ...
    {
        public bool CreateModel(string model, string cusmodel, string cusID, string gerber, string verBom, string opcontact, string phone, string potential, string possibility, string piority, bool IATF)
        {
            int isIATF = IATF == true ? 1 : 0;
            string sql = $"INSERT INTO `TRACKING_SYSTEM`.`DEFINE_MODEL` (`MODEL_ID`, `CUS_MODEL`, `CUS_ID`, `VER_GERBER`, `VER_BOM`, `OP_CONTACT`, `PHONE`, `POTENTIAL`, `POSSIBILITY`, `PIORITY`,`IATF`)" +
                $" VALUES ('{model}', '{cusmodel}', '{cusID}', '{gerber}', '{verBom}', '{opcontact}', '{phone}', '{potential}', '{possibility}', '{piority}','{isIATF}')" +
                $" ON DUPLICATE KEY UPDATE `CUS_MODEL` = '{cusmodel}', `CUS_ID` =  '{cusID}' , `VER_GERBER` = '{gerber}', `VER_BOM` = '{verBom}', " +
                $"`OP_CONTACT` = '{opcontact}', `PHONE` = '{phone}', `POTENTIAL` = '{potential}', `POSSIBILITY` = '{possibility}', `PIORITY`  =  '{piority}', IATF='{isIATF}';";
            if (mySQL.InsertDataMySQL(sql)) return true;
            return false; 
        }


        public bool SaveModelChild(string model, string cusmodel, string process, string cusID, int pcsOnPanel, string modelID, string modelPart, string gerber, string bom)
        {
            string sql = $"INSERT INTO `TRACKING_SYSTEM`.`PART_MODEL_CONTROL` (`ID_MODEL`, `MODEL_NAME`, `PROCESS`, `CUSTOMER_ID`, `PCBS`, `MODEL_ID`, `MODEL_PARENT`, `VER_GERBER`, `VER_BOM`) VALUES " +
                $"('{model}', '{cusmodel}', '{process}', '{cusID}', '{pcsOnPanel}' , '{modelID}', '{modelPart}', '{gerber}', '{bom}')" +
                $" ON DUPLICATE KEY UPDATE  `MODEL_NAME` = '{cusmodel}', `PROCESS` = '{process}' , `CUSTOMER_ID` = '{cusID}',`PCBS` = '{pcsOnPanel}', `VER_GERBER` = '{gerber}', `VER_BOM` = '{bom}';";
            if (mySQL.InsertDataMySQL(sql)) return true;
            return false;
        }
