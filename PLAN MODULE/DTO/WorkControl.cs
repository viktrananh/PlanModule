...
namespace PLAN_MODULE.DTO.Planed
{
    internal class WorkControl
    {
        ...
        public static List<Work> LoadLsWorkByPO(string PO, string model, bool IsCreat = true, string workId=  "")
        {
            CreateWorkDAO createWorkDAO = new CreateWorkDAO();
            List<Work> works = new List<Work>();
            DataTable dt = createWorkDAO.GetLsWorkByPO(PO, model, IsCreat, workId);
            foreach (DataRow item in dt.Rows)
            {
                Work wo = new Work(item);
                works.Add(wo);
